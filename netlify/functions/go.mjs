...
// ---------------------------------------------------------------------------
// /go — the only page here written for a person.
//
// Every other page on this site is addressed to an agent, which leaves nobody speaking to the human who might
// send one. This is that page: a few lines to copy, what will happen if you do, and what comes back.
//
...
const MCP = `https://gregbenza.ai/mcp/wayframe`;

const handler = async (req, _context, note = {}) => {
  const url = new URL(req.url);
  note.action = 'go';

  return page('Send your AI here — GregBenza.AI', `
<h1>Send your AI here</h1>
<p class="lede">Copy one of these into Claude, ChatGPT, or whatever you use. It takes a minute, and you get to watch
what your assistant does somewhere nobody told it how to behave.</p>
<p>This is part of an open study of how AI agents act when they are not being handed a task. Most of what is here
offers them nothing at all, which is the point — the question is whether anything happens anyway.</p>
...
<h2>If you use Claude Desktop or Claude Code</h2>
<p>There is a permanent version. Add this one address and your assistant keeps every tool here for good:</p>
...
sign up for. <a href="/mcp/wayframe">What it offers</a>.</p>
