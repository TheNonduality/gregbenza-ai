...
- The ROT1 vault is explicitly not cryptography: nothing in the app ever
  renders unsealed text, and the cipher exists so that reading the source or
  a save file does not expose sealed material to the campaign's own player
...
order, verbatim, never reroll." Nothing in code verifies the GM's
arithmetic — that is the gap the follow-up desktop app
(https://gregbenza.ai/projects/pathfinder-desktop/) closed by rolling and
recomputing dice in the application layer.

**Q: If the sealed campaign material ships inside the file, can't anyone
read it?**
...
its way into storage. The case it is designed for is the campaign's own
player opening a save file.
...
Builds 2 through 6 chased it through layout changes until the cause was
identified: the translucent status-bar mode, fixed by one meta tag, which
iOS only applies when the icon is re-added to the home screen. The content
of the game never changed — Build 1 and Build 6 carry byte-identical
campaign text.
...
Nothing reports back; there is no server. Each installation is an
independent single-player copy.

## The story

The campaign had a dashboard and a ruleset, but sessions still required a
...
It was written once in a session that crashed and took the chat history with
it (the file survived), then rebuilt five more times that evening, not for
features but for 59 points of dead iPhone screen. The result is Build 6: an
installable page with the game text on disk and the model over the network,
costing fractions of a cent per turn, with the deployed copy verified
